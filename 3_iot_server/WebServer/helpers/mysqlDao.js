...
MySqlDao.prototype.searchor = function(wherejson, orwherejson, cb) {
  var that = this;
  wherejson = utils.decamelizeOjbect(wherejson);
  orwherejson = utils.decamelizeOjbect(orwherejson);
...
MySqlDao.prototype.addOrUpdateWithSid = function(recordjson, sidFieldName, cb) {
  var self = this;
  recordjson = utils.decamelizeOjbect(recordjson);
...
MySqlDao.prototype.updateWhere = function(wherejson, updatejson, cb) {
  var that = this;
  wherejson = utils.decamelizeOjbect(wherejson);
  updatejson = utils.decamelizeOjbect(updatejson);
  ...
  this.rawknex(that.table)
    ...
    .then(function(count) {
      ...
      return cb(null, camelize(updatejson));
    })
    .catch(function(err) {
      console.error(err);
      if( cb ) {
